...
static TOUCHPANEL *gpio2tch[48];//48
...
TOUCHPANEL::TOUCHPANEL(gpio_num_t scl, gpio_num_t sdo) {
    ...
    gpio_config(&conf);
    conf.intr_type = GPIO_INTR_NEGEDGE;//pino sai do estado 1 para o estado 0
    ...
    queue_touch = xQueueCreate(10, sizeof(uint16_t));
...
void TOUCHPANEL::interrupt(gpio_num_t gpio) {
    if(gpio == sdo)
    {
      ...
      xQueueSendFromISR(queue_touch, &bitmap, NULL);
